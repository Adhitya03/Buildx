 ...
 * located in the assets/display-comments.php file.
 *
 * @package Shape
 * @since Shape 1.0
 */
...
<div id="comments" class="col-12">
		...
		<div class="card card-comments mb-3 wow fadeIn">
			...
			<?php if ( get_comment_pages_count() > 1 && get_option( 'page_comments' ) ) : // are there comments to navigate through? If so, show navigation ?>
				<nav role="navigation" id="comment-nav-above" class="site-navigation comment-navigation text-center mb-2">
					...
					<?php previous_comments_link( __( '<i class="fa fa-caret-left left" aria-hidden="true"></i> Older Comments', 'buildx' ) ); ?>

					<?php next_comments_link( __( 'Newer Comments <i class="fa fa-caret-right right" aria-hidden="true"></i>', 'buildx' ) ); ?>
	...
	if ( ! comments_open() && '0' != get_comments_number() && post_type_supports( get_post_type(), 'comments' ) ) :
		?>
		<p class="nocomments">
			<?php esc_html_e( 'Comments are closed.', 'buildx' ); ?>
		</p>
	<?php endif; ?>
	<?php
	...
	$args = array(
		'fields' => apply_filters(
			'comment_form_default_fields', array(
				'author' =>'
                                                ...
                                                <!-- Name -->
                                                <label for="name">'. __( 'Your name', 'buildx' ) . ( $req ? '<span class="required">*</span>' : '' )  .' </label>
				...
				'email'  => '
                                                <!-- Email -->
                                                <label for="email">'. __( 'Your email', 'buildx' ) . ( $req ? '<span class="required">*</span>' : '' ). '</label>
		...
		'logged_in_as' => '<p class="text-center">('. sprintf(
				__( 'Logged in as <a href="%1$s">%2$s</a>. <a href="%3$s" title="Log out of this account">Log out?</a>', 'buildx' ),
	...
	<div class="card mb-3 wow fadeIn">
		<div class="card-header">Leave a reply on <?php the_title(); ?></div>
		<div class="card-body">
